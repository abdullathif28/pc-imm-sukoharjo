...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('super-admin.konten.create') }}" class="btn btn-primary">
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('super-admin.konten.index') }}" class="row g-2">
            <div class="col-md-3">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                ...
                                @else
                                    <div class="rounded d-flex align-items-center justify-content-center text-white" style="width:50px;height:50px;background:{{ $konten->bidang->warna ?? '#6c757d' }}">
                                        <i class="bi bi-image"></i>
                                    </div>
                                @endif
                                <div>
                                    <div class="fw-semibold">{{ Str::limit($konten->judul, 50) }}</div>
                                    <small class="text-muted">{{ $konten->user->name }}</small>
                                </div>
                            </div>
                        </td>
                        <td>
                            <span class="badge" style="background:{{ $konten->bidang->warna ?? '#6c757d' }}">{{ $konten->bidang->singkatan }}</span>
                        </td>
                        <td>
                            <span class="badge bg-light text-dark border">{{ $konten->jenis_label }}</span>
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-1">
                                ...
                                <a href="{{ route('super-admin.konten.edit', $konten) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <form action="{{ route('super-admin.konten.destroy', $konten) }}" method="POST" onsubmit="return confirm('Hapus konten ini?')">
                                    @csrf @method('DELETE')
